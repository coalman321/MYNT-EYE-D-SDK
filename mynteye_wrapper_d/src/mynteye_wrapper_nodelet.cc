 ...
 public:
    ...
    MYNTEYEWrapperNodelet() : Node("mynteye_wrapper_d") {
        ...
        RCLCPP_INFO_STREAM(this->get_logger(),
                           "left_mono_frame: " << left_mono_frame_id);
        RCLCPP_INFO_STREAM(this->get_logger(),
                           "left_color_frame: " << left_color_frame_id);
        RCLCPP_INFO_STREAM(this->get_logger(),
                           "right_mono_frame: " << right_mono_frame_id);
        RCLCPP_INFO_STREAM(this->get_logger(),
                           "right_color_frame: " << right_color_frame_id);
        RCLCPP_INFO_STREAM(this->get_logger(),
                           "depth_frame: " << depth_frame_id);
        RCLCPP_INFO_STREAM(this->get_logger(),
                           "points_frame: " << points_frame_id);
        RCLCPP_INFO_STREAM(this->get_logger(), "imu_frame: " << imu_frame_id);
        RCLCPP_INFO_STREAM(this->get_logger(), "temp_frame: " << temp_frame_id);
        RCLCPP_INFO_STREAM(this->get_logger(),
        ...
        // points
        pub_points = this->create_publisher<sensor_msgs::msg::PointCloud2>(points_topic, 1);
        RCLCPP_INFO_STREAM(this->get_logger(),
        ...
        // temp
        pub_temp = this->create_publisher<sensor_msgs::msg::Temperature>(temp_topic, 100);
        RCLCPP_INFO_STREAM(this->get_logger(),
        ...
        // loop
        pub_timer = this->create_wall_timer(framerate, std::bind(this, MYNTEYEWrapperNodelet::timerCallback));
    }
    ...
    void openDevice() {
            ...
            mynteye->SetStreamCallback(type, [this](const StreamData& data) {
                ...
                switch (data.img->type()) {
                    ...
                    case ImageType::IMAGE_RIGHT_COLOR: {
                        if (sub_result_right) {
                            publishRight(data, timestamp,
                                         sub_result_right_color,
        ...
        mynteye->SetMotionCallback([this](const MotionData& data) {
            ...
            pthread_mutex_unlock(&mutex_sub_result);
            if (data.imu && (sub_result_imu || sub_result_imu_processed ||
                             sub_result_temp)) {
                if (data.imu->flag == MYNTEYE_IMU_ACCEL) {
                    imu_accel = data.imu;
                    if (imu_timestamp_align) {
                        publishAlignImu(sub_result_imu,
        ...
        RCLCPP_INFO_STREAM(this->get_logger(), "Open camera success");

        // camera infos
        bool in_ok;
        auto&& in = mynteye->GetStreamIntrinsics(params.stream_mode, &in_ok);
        if (in_ok) {
            RCLCPP_INFO_STREAM(this->get_logger(), "Camera info is created");
        } else {
    ...
    void publishRight(const StreamData& data, const ros::Time& timestamp,
        ...
        publishColor(data, timestamp, right_info_ptr, pub_right_color,
                     color_sub, right_color_frame_id, pub_right_mono, mono_sub,
                     right_mono_frame_id, false);
    }

    void publishColor(const StreamData& data, const ros::Time& timestamp,
    ...
    void publishDepth(const StreamData& data, const ros::Time& timestamp) {
        ...
        if (depth_type == 0) {
            ...
            pub_depth.publish(
                cv_bridge::CvImage(header, enc::TYPE_16UC1, mat).toImageMsg(),
                info);
        }
        pthread_mutex_lock(&mutex_sub_result);
        bool sub_result_points = sub_result.points;
    ...
    void timestampAlign() {
        ...
        if (imu_accel != nullptr) {
            ...
            if (acc.timestamp <= gyro.timestamp) {
                ...
                double k =
                    static_cast<double>(imu_accel->timestamp - acc.timestamp);
    ...
    void publishAlignImu(bool imu_sub, bool imu_processed_sub, bool temp_sub) {
        ...
        if (motion_intrinsics_enabled && imu_processed_sub) {
            ...
            auto msg = getImuMsgFromData(
                ros::Time::now(), imu_frame_processed_id, data_acc2, data_gyr2);
            msg.header.stamp = stamp;
        ...
        if (temp_sub) {
            auto msg =
                getTempMsgFromData(ros::Time::now(), temp_frame_id, *imu_accel);
            msg.header.stamp = stamp;
    ...
    void publishImu(bool imu_sub, bool imu_processed_sub, bool temp_sub) {
        ...
        if (imu_sub) {
            auto msg = getImuMsgFromData(this->get_clock()->now(), imu_frame_id,
                                         *imu_accel, *imu_gyro);
            msg.header.stamp = stamp;
            pub_imu->publish(msg);
        }
    ...
    sensor_msgs::msg::CameraInfo::ConstPtr createCameraInfo(const CameraIntrinsics& in) {
        ...
        auto camera_info_ptr = sensor_msgs::msg::CameraInfo::ConstPtr(camera_info);
    ...
    rclcpp::Time hardTimeToSoftTime(std::uint64_t _hard_time) {
        ...
        if (false == isInited) {
            soft_time_begin = this->get_clock()->now().seconds();
        ...
        double time_sec_double =
            rclcpp::Time(time_ns_detal_s, time_ns_detal_ns * 10000).seconds();
    ...
    rclcpp::Time checkUpTimeStamp(std::uint64_t _hard_time,
        ...
        return hardTimeToSoftTime(image_acc[type] * unit_hard_time +
                                  _hard_time);
    }
    ...
    void publishMesh() {
        ...
        mesh_msg_.header.stamp = this->get_clock()->now();
    ...
    bool getParams(const std::shared_ptr<mynteye_wrapper_d::srv::GetParams::Request> req,     // NOLINT
                   std::shared_ptr<mynteye_wrapper_d::srv::GetParams::Response> res) {  // NOLINT
        ...
        switch (req->key) {
            case Request::IMG_INTRINSICS: {
                bool in_ok;
                auto&& in_vga = mynteye->GetStreamIntrinsics(
                    StreamMode::STREAM_1280x480, &in_ok_1);  // NOLINT
                auto&& in_hd = mynteye->GetStreamIntrinsics(
                    StreamMode::STREAM_2560x720, &in_ok_2);  // NOLINT
                if (params.stream_mode == StreamMode::STREAM_1280x480 &&
                    in_ok_1) {
                    Config intrinsics{
                        {"calib_model", "pinhole"},
                        {"left",
                         {
                             ...
                             {"coeffs",
                              Config::array(
                                  {in_vga.left.coeffs[0], in_vga.left.coeffs[1],
                                   in_vga.left.coeffs[2], in_vga.left.coeffs[3],
                                   in_vga.left.coeffs[4]})},
                             {"p", Config::array(
                                       {in_vga.left.p[0], in_vga.left.p[1],
                        ...
                        {"left",
                         {
                             ...
                             {"coeffs",
                              Config::array(
                                  {in_hd.left.coeffs[0], in_hd.left.coeffs[1],
                                   in_hd.left.coeffs[2], in_hd.left.coeffs[3],
                                   in_hd.left.coeffs[4]})},
                             {"p",
                              Config::array({in_hd.left.p[0], in_hd.left.p[1],
                        ...
                        {"right",
                         {
                             ...
                             {"coeffs",
                              Config::array(
                                  {in_hd.right.coeffs[0], in_hd.right.coeffs[1],
                                   in_hd.right.coeffs[2], in_hd.right.coeffs[3],
                                   in_hd.right.coeffs[4]})},
                             {"p", Config::array(
                                       {in_hd.right.p[0], in_hd.right.p[1],
            ...
            case Request::IMG_EXTRINSICS_RTOL: {
                ...
                if (params.stream_mode == StreamMode::STREAM_1280x480 &&
                    ...
                    Config extrinsics{
                        {"rotation",
                         Config::array(
                             {vga_extrinsics.rotation[0][0],
                              vga_extrinsics.rotation[0][1],
                              vga_extrinsics.rotation[0][2],  // NOLINT
                              vga_extrinsics.rotation[1][0],
                              vga_extrinsics.rotation[1][1],
                              vga_extrinsics.rotation[1][2],  // NOLINT
                              vga_extrinsics.rotation[2][0],
                              vga_extrinsics.rotation[2][1],
                              vga_extrinsics.rotation[2][2]})},  // NOLINT
                        {"translation",
                         Config::array(
                             {vga_extrinsics.translation[0],
                    ...
                    Config extrinsics{
                        {"rotation",
                         Config::array(
                             {hd_extrinsics.rotation[0][0],
                              hd_extrinsics.rotation[0][1],
                              hd_extrinsics.rotation[0][2],  // NOLINT
                              hd_extrinsics.rotation[1][0],
                              hd_extrinsics.rotation[1][1],
                              hd_extrinsics.rotation[1][2],  // NOLINT
                              hd_extrinsics.rotation[2][0],
                              hd_extrinsics.rotation[2][1],
                              hd_extrinsics.rotation[2][2]})},  // NOLINT
                        {"translation",
                         Config::array(
                             {hd_extrinsics.translation[0],
                        ...
                        {"accel",
                         {
                             {"scale",
                              Config::array(
                                  {intri.accel.scale[0][0],
                                   intri.accel.scale[0][1],
                                   intri.accel.scale[0][2],  // NOLINT
                                   intri.accel.scale[1][0],
                                   intri.accel.scale[1][1],
                                   intri.accel.scale[1][2],  // NOLINT
                                   intri.accel.scale[2][0],
                                   intri.accel.scale[2][1],
                                   intri.accel.scale[2][2]})},  // NOLINT
                             {"assembly",
                              Config::array(
                                  {intri.accel.assembly[0][0],
                                   intri.accel.assembly[0][1],
                                   intri.accel.assembly[0][2],  // NOLINT
                                   intri.accel.assembly[1][0],
                                   intri.accel.assembly[1][1],
                                   intri.accel.assembly[1][2],  // NOLINT
                                   intri.accel.assembly[2][0],
                                   intri.accel.assembly[2][1],
                                   intri.accel.assembly[2][2]})},  // NOLINT
                             {"drift",
                              Config::array({intri.accel.drift[0],
                                             intri.accel.drift[1],
                                             intri.accel.drift[2]})},  // NOLINT
                             {"noise",
                              Config::array({intri.accel.noise[0],
                                             intri.accel.noise[1],
                                             intri.accel.noise[2]})},  // NOLINT
                             {"bias",
                              Config::array({intri.accel.bias[0],
                                             intri.accel.bias[1],
                                             intri.accel.bias[2]})},  // NOLINT
                             {"x",
                              Config::array({intri.accel.x[0],
                                             intri.accel.x[1]})},  // NOLINT
                             {"y",
                              Config::array({intri.accel.y[0],
                                             intri.accel.y[1]})},  // NOLINT
                             {"z", Config::array({intri.accel.z[0],
                                                  intri.accel.z[1]})}  // NOLINT
                         }},
                        {"gyro",
                         {
                             {"scale",
                              Config::array(
                                  {intri.gyro.scale[0][0],
                                   intri.gyro.scale[0][1],
                                   intri.gyro.scale[0][2],  // NOLINT
                                   intri.gyro.scale[1][0],
                                   intri.gyro.scale[1][1],
                                   intri.gyro.scale[1][2],  // NOLINT
                                   intri.gyro.scale[2][0],
                                   intri.gyro.scale[2][1],
                                   intri.gyro.scale[2][2]})},  // NOLINT
                             {"assembly",
                              Config::array(
                                  {intri.gyro.assembly[0][0],
                                   intri.gyro.assembly[0][1],
                                   intri.gyro.assembly[0][2],  // NOLINT
                                   intri.gyro.assembly[1][0],
                                   intri.gyro.assembly[1][1],
                                   intri.gyro.assembly[1][2],  // NOLINT
                                   intri.gyro.assembly[2][0],
                                   intri.gyro.assembly[2][1],
                                   intri.gyro.assembly[2][2]})},  // NOLINT
                             {"drift",
                              Config::array({intri.gyro.drift[0],
                                             intri.gyro.drift[1],
                                             intri.gyro.drift[2]})},  // NOLINT
                             {"noise",
                              Config::array({intri.gyro.noise[0],
                                             intri.gyro.noise[1],
                                             intri.gyro.noise[2]})},  // NOLINT
                             {"bias",
                              Config::array({intri.gyro.bias[0],
                                             intri.gyro.bias[1],
                                             intri.gyro.bias[2]})},  // NOLINT
                             {"x", Config::array({intri.gyro.x[0],
                                                  intri.gyro.x[1]})},  // NOLINT
                             {"y", Config::array({intri.gyro.y[0],
                                                  intri.gyro.y[1]})},  // NOLINT
                             {"z", Config::array({intri.gyro.z[0],
                                                  intri.gyro.z[1]})}  // NOLINT
                         }}};
                    std::string json = dump_string(intrinsics, JSON);
                    res->value = json;
                } else {
                    RCLCPP_INFO_STREAM(this->get_logger(),
                                       "INVALID IMU INTRINSICS");
                    res->value = "null";
                }
            } break;
            case Request::IMU_EXTRINSICS: {
                ...
                if (is_ok) {
                    Config extrinsics{
                        {"rotation",
                         Config::array(
                             {extri.rotation[0][0], extri.rotation[0][1],
                        ...
                        {"translation",
                         Config::array({extri.translation[0],
                                        extri.translation[1],
